...
def launch_setup(context: LaunchContext) -> list:
    ...
    # Ensure horizon mesh includes an explicit scale in the URDF.
    # Some SDF->URDF generators omit mesh scale attributes; inject one
    # for the horizon model so visualization matches the SDF/DAE scaling.
    if "horizon.dae" in robot_urdf_xml:
        # Inject scale="0.01 0.01 0.01" into mesh tags that reference horizon.dae
        # This is a conservative replacement that adds the attribute to occurrences
        # like: filename=".../horizon.dae" -> filename=".../horizon.dae" scale="0.01 0.01 0.01"
        robot_urdf_xml = robot_urdf_xml.replace('horizon.dae"', 'horizon.dae" scale="0.001 0.001 0.001"')
